...
export default function DatabasePage() {
  ...
  const loadRows = useCallback(async (tableKey: string) => {
    ...
    setRowsLoading(true);
    try {
      const res = await fetch(`/api/db/tables/${encodeURIComponent(tableKey)}/rows?limit=100`, {
        cache: "no-store",
      });
  ...
  return (
    <section className="panel">
      <h1>Database</h1>
      <p className="hint">
        Supervisor health from <code>lis db status</code> and read-only table browser via lidb catalog.
      </p>
      <p>
      ...
      <div className="split-pane">
        <ul className="table-list">
          {tables.length === 0 ? (
            <li className="hint">No catalog tables (is lidb running?)</li>
          ) : (
            tables.map((t) => (
              <li key={t.key}>
                <button
