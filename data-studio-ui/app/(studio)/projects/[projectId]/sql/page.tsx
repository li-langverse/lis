...
export default function ProjectSqlPage() {
  ...
  return (
    <section className="panel">
      <h1>SQL Editor</h1>
      <p className="hint">Read-only SELECT queries against this project lidb (max 500 rows).</p>
      <textarea
        className="sql-input"
        value={sql}
      ...
      {result?.ok ? <p className="hint">{result.row_count ?? result.rows?.length ?? 0} row(s)</p> : null}
      {result?.columns ? (
