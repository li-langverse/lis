...
function lisRoot(): string {
  return process.env.LIS_ROOT ?? path.resolve(process.cwd(), "..");
}

function lisBin(): string {
  const root = lisRoot();
  const win = process.platform === "win32";
  return path.join(root, win ? "bin/lis" : "bin/lis");
}

export async function GET() {
  const bin = lisBin();
  const shell = process.env.LIS_DB_STATUS_SHELL ?? (process.platform === "win32" ? "bash" : undefined);
