#!/usr/bin/env bash
# Multi-peer E2E on engine cluster: registry via liserver (:80), two peer seeders, parallel fetch.
set -euo pipefail
...
# li-httpd PUT proxy can empty-reply on body uploads; writes go direct to registry nodeport.
WRITE_BASE="${LIP_REGISTRY_WRITE_BASE:-http://127.0.0.1:30422}"
...
echo "== multipeer-e2e: edge liserver ${REGISTRY_HOST}:${REGISTRY_PORT} Host=${HOST_HDR} =="
...
echo "edge: li-httpd liserver on :${REGISTRY_PORT} (public :80 via socat; nginx/caddy stopped)"
...
echo "== PUT blob $digest (direct registry; reads via liserver) =="
curl -fsS --max-time 30 -X PUT \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/vnd.li.package+tar" \
  --data-binary "$payload" \
  "${WRITE_BASE}/v1/blobs/${digest}"
...
curl -fsS --max-time 30 -X POST \
  ...
  "${WRITE_BASE}/v1/packages/${PKG}/versions"

echo "== announce peers =="
for ep in "$PEER_A" "$PEER_B"; do
  python3 - "${WRITE_BASE}/v1" "$ep" "$digest" "$TOKEN" <<'PY'
...
pkg_json="$(curl -fsS --max-time 30 "${WRITE_BASE}/v1/packages/${PKG}/${VER}")"
echo "$pkg_json"
python3 - "$pkg_json" <<'PY'
import json, sys
...
peers_json="$(curl -fsS --max-time 30 "${WRITE_BASE}/v1/peers?digest=${digest}")"
echo "$peers_json"
...
export REGISTRY_HOST REGISTRY_PORT HOST_HDR WRITE_BASE
...
want = sys.argv[2]
digest = sys.argv[3]
write_base = os.environ.get("WRITE_BASE", "http://127.0.0.1:30422").rstrip("/")
sources = pkg.get("sources") or []

def fetch(src: dict) -> bytes:
    url = src["url"]
    if src.get("type") == "origin" or url.startswith("/"):
        full = f"{write_base}{url}" if url.startswith("/") else f"{write_base}/v1/blobs/{digest}"
